...
class BeestoWorkflowModelStaffNotes extends JModelList
{
	...
	protected function getListQuery() {
		...
		$query->select(
			$this->getState(
				'list.select','a.id,a.title,a.added,a.category'
			)
		);
		$query->from(' #__beestowf_notes AS a ');
		...
		if($status ) {
			$query->where('a.category = '. $db->Quote($status));
		}
